...
interface VentaDao {
    ...
    @Query("SELECT SUM(cantidad * precioUnitario) FROM ventas WHERE fechaVenta BETWEEN :inicio AND :fin")
    fun getTotalVendidoEnRango(inicio: Long, fin: Long): Flow<Double?>

    @Query("SELECT SUM(cantidad) FROM ventas WHERE fechaVenta BETWEEN :inicio AND :fin")
